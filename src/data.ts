...
export const defaultPortfolioData: PortfolioData = {
  ...
  impact: [
    ...
    },
    {
      ...
      label: "成本降低",
      description:
        "多模态 Token 压缩 + 缓存策略，使 AI 运行成本降低 40%（体验与稳定性并重）",
      details: [
  ...
  timeline: [
    ...
    },
    {
      ...
      expandedDetails: {
        background:
          "原有基于同步框架的微信机器人无法处理并发请求，且长对话中 Token 消耗过快。",
        problem:
          "主线程阻塞导致消息丢失；大文件日志读取导致内存溢出；缺乏长期记忆能力。",
        solution:
          "- **异步化**：使用 `asyncio.to_thread` 将 wxauto 阻塞调用剥离，基于 SSE 推送实时状态。\n- **记忆系统**：构建 RAG 检索增强生成链路，利用向量数据库存储长期记忆。\n- **资源优化**：实现日志流式读取与 LRU 缓存，优化图片压缩策略。",
        result:
          "实现 7x24 小时稳定运行，支持多轮复杂对话与图片识别，API 响应速度提升 5 倍。",
        role: "独立开发者",
        techStack: [
          "Python",
          "Quart",
  ...
  projects: [
    ...
    },
    {
      ...
      summary:
        "微信 AI 助手：异步架构提升并发能力，RAG 提供长期记忆，Electron 管理端便于运维（成本与体验兼顾）。",
      impact: "异步架构 + RAG 记忆",
      keyOutcomes: [
