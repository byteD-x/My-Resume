...
const sendToGA = (metric: MetricPayload) => {
    ...
    if (process.env.NODE_ENV === 'development') {
        const marker =
            metric.rating === 'good' ? 'OK' : metric.rating === 'needs-improvement' ? 'WARN' : 'POOR';
        const normalized = metric.name === 'CLS' ? metric.value.toFixed(3) : `${metric.value.toFixed(2)}ms`;
...
export function usePerformanceMonitor(componentName: string) {
    useEffect(() => {
        ...
        return () => {
            ...
            if (measure) {
                console.log(`[component-perf] ${componentName}: ${(endTime - startTime).toFixed(2)}ms`);
            }
            ...
            const observer = new PerformanceObserver((list) => {
                for (const entry of list.getEntries()) {
                    if (entry.duration > 50) {
                        console.warn(`[perf-budget] Long task detected: ${entry.duration.toFixed(2)}ms`, entry);
                    }
                }
            });
