...
function buildDerivedBasis(verification: VerificationInfo, daysSince: number | null): string[] {
    const basis: string[] = [];
    ...
    if (daysSince === null) {
        basis.push(`验证时间：${verification.verifiedAt}（格式异常，无法计算距今天数）`);
    } else if (daysSince <= 180) {
        basis.push(`验证时间：${verification.verifiedAt}（距今 ${daysSince} 天，时效性高）`);
    } else if (daysSince <= 365) {
        basis.push(`验证时间：${verification.verifiedAt}（距今 ${daysSince} 天，时效性可接受）`);
    } else {
        basis.push(`验证时间：${verification.verifiedAt}（距今 ${daysSince} 天，建议更新复核）`);
    }

    return basis;
}

function buildDerivedReason(verification: VerificationInfo, daysSince: number | null): string {
...
export function getConfidenceDisplayText(confidence: VerificationInfo['confidence']): string {
