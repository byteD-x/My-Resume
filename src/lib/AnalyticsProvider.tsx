...
import { initGA, trackPageView, trackScrollDepth, trackSectionView } from './analytics';
import { getScrollObserverSnapshot, subscribeScrollObserver } from './scroll-observer';
...
export function AnalyticsProvider({ children }: AnalyticsProviderProps) {
    ...
    useEffect(() => {
        ...
        const trackMilestones = () => {
            ...
            const scrollPercent = Math.round(getScrollObserverSnapshot().progress * 100);
            [25, 50, 75, 100].forEach((milestone) => {
                if (scrollPercent < milestone || scrollMilestonesRef.current.has(milestone)) return;
