...
const normalizeTelemetry = (payload: unknown): GitHubTelemetryPayload => {
    ...
    const repos = Array.isArray(record.specificRepos)
        ? record.specificRepos
              .map((repo) => {
                  ...
                  const url = toStringOrFallback(item.url, `https://github.com/${name}`);
    ...
    const source =
        record.source === 'github-api' || record.source === 'github-web' || record.source === 'fallback'
            ? record.source
            : 'fallback';
    ...
    return {
        ...
        message: toStringOrFallback(record.message, fallbackGithubTelemetry.message),
    };
};
...
export async function getGithubTelemetry(options?: { force?: boolean }): Promise<GitHubTelemetryPayload> {
