...
const extractNumberFromAnchor = (html: string, hrefPattern: string): number | null => {
...
const fetchUserStatsFromWeb = async (): Promise<Pick<TelemetryPayload, 'followers' | 'public_repos'> | null> => {
    const html = await fetchText(`https://github.com/${GITHUB_USERNAME}`);
    if (!html) return null;

    const followers = extractNumberFromAnchor(html, `/${GITHUB_USERNAME}\\?tab=followers`);
    const publicRepos = extractNumberFromAnchor(html, `/${GITHUB_USERNAME}\\?tab=repositories`);
...
export async function GET() {
    ...
    try {
        const [userRes, reposRes] = await Promise.all([
            ...
            fetchWithTimeout(`https://api.github.com/users/${GITHUB_USERNAME}/repos?per_page=100`, {
                headers: githubApiHeaders(),
                next: { revalidate },
            }),
        ]);
        ...
        if (userRes.ok && reposRes.ok) {
            ...
            const specificRepos = REPO_NAMES.reduce<GitHubRepoStat[]>((acc, repoName) => {
