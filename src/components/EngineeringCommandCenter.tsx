...
const deployTarget = process.env.NEXT_PUBLIC_DEPLOY_TARGET ?? (isStaticExport ? 'static-export' : 'server');
                            ...
                            <header className="sticky top-0 z-10 border-b border-slate-200 bg-white px-6 py-4">
                                <div className="flex items-center justify-between">
                                    <div>
                                        ...
                                        <p className="text-sm text-slate-500">实时指标 / 开源数据 / 工程指纹</p>
                                    </div>
                                    <button
                                        ref={closeButtonRef}
                                        onClick={() => setIsOpen(false)}
                                ...
                                <section>
                                    ...
                                    <div className="grid grid-cols-3 gap-3">
                                        <article className="rounded-xl border border-slate-200 p-4">
                                            ...
                                            <p className="mt-1 text-xl font-bold text-slate-900">{formatCount(githubTelemetry.followers)}</p>
                                        </article>
                                        <article className="rounded-xl border border-slate-200 p-4">
                                            <p className="text-xs text-slate-500">仓库数</p>
                                            <p className="mt-1 text-xl font-bold text-slate-900">{formatCount(githubTelemetry.public_repos)}</p>
                                        </article>
                                        <article className="rounded-xl border border-slate-200 p-4">
                                            <p className="text-xs text-slate-500">总 Star</p>
                                            <p className="mt-1 text-xl font-bold text-slate-900">{formatCount(githubTelemetry.totalStars)}</p>
                                        </article>
                                    </div>
                                    <div className="mt-3 rounded-xl border border-slate-200 p-4">
                                        <div className="mb-2 flex flex-wrap items-center justify-between gap-2">
                                            <p className="text-xs font-medium text-slate-500">关注仓库</p>
                                        ...
                                        <ul className="mt-2 space-y-2">
                                            ...
                                            {githubTelemetry.specificRepos.map((repo) => (
                                                <li key={repo.name} className="flex items-center justify-between text-sm">
                                                    ...
                                                    <span className="text-slate-900">★ {repo.stars.toLocaleString('en-US')}</span>
                                                </li>
                                            ))}
                                        </ul>
                                        <p className="mt-3 text-xs text-slate-500">
                                            {telemetryState === 'loading' && '正在从 GitHub 获取实时数据...'}
                                            {telemetryState === 'error' && (githubTelemetry.message ?? '数据拉取失败。')}
                                            {telemetryState === 'ready' && githubTelemetry.message}
                                            {githubTelemetry.isPartial && telemetryState === 'ready' && '（部分数据）'}
                                        </p>
                                    </div>
                                </section>
                                    ...
                                    <div className="rounded-xl border border-slate-200 p-4">
                                        <ul className="space-y-2 text-sm text-slate-700">
                                            <li className="flex items-center justify-between">
                                                <span>Next.js</span>
                                                <span className="font-semibold text-slate-900">{nextVersion}</span>
                                            </li>
                                            <li className="flex items-center justify-between">
                                                <span>React</span>
                                                <span className="font-semibold text-slate-900">{reactVersion}</span>
                                            </li>
                                            <li className="flex items-center justify-between">
                                                <span>TypeScript</span>
                                                <span className="font-semibold text-slate-900">{tsVersion}</span>
                                            </li>
                                            <li className="flex items-center justify-between">
                                                <span>部署模式</span>
                                            ...
                                            <li className="flex items-center justify-between">
                                                <span>构建时间</span>
                                                <span className="font-semibold text-slate-900">{buildTimestamp}</span>
                                            </li>
                                        </ul>
                                    </div>
                                </section>
