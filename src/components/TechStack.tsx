...
const DEFAULT_VISIBLE_SKILL_ITEMS = 3;
  ...
  return (
    <Section className="theme-grid-section theme-section-balanced relative border-y section-divider">
      <Container>
        ...
        <div className="grid grid-cols-1 gap-3 lg:gap-4 xl:grid-cols-3 sm:gap-4">
              ...
              >
                <div className="mb-3.5 flex items-start gap-3 border-b border-[color:var(--border-default)] pb-3 sm:mb-5 sm:gap-3.5 sm:pb-4">
                  ...
                  <div className="flex flex-col pt-0.5">
                    <h3 className="theme-card-title text-[1rem] sm:text-[1.02rem]">
                      {category.category}
                    </h3>
                    {category.description ? (
                      <p className="theme-card-body mt-1 text-[12px] sm:mt-1.5 sm:text-[13px]">
                        {category.description}
                      </p>
                    ) : null}
                  </div>
                ...
                <div className="grid auto-rows-fr gap-1.5 sm:grid-cols-2 sm:gap-2.5">
                  {displayItems.map((item) => (
                    <article
                      key={item.raw}
                      className="theme-card-muted flex h-full flex-col rounded-[0.95rem] p-2.5"
                    >
                      <p className="theme-title text-[12px] font-semibold leading-5 sm:text-[13px] sm:leading-6">
                        {item.label}
                      </p>
                      {item.detail ? (
