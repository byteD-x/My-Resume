...
function detectLowPerformanceMode(): boolean {
    ...
    const lowCoreCount =
        typeof nav.hardwareConcurrency === 'number' && nav.hardwareConcurrency <= DEVICE_HINTS.LOW_CORE_COUNT;
    const lowMemory =
        typeof nav.deviceMemory === 'number' && nav.deviceMemory <= DEVICE_HINTS.LOW_MEMORY_GB;
...
const startListening = () => {
    ...
    mediaQueries = LOW_PERF_MEDIA_QUERIES.map((query) => window.matchMedia(query));
    mediaQueries.forEach((mediaQuery) => mediaQuery.addEventListener('change', syncSnapshot));
...
const stopListening = () => {
    ...
    mediaQueries.forEach((mediaQuery) => mediaQuery.removeEventListener('change', syncSnapshot));
    mediaQueries = [];
