...
const DEGRADED_HOSTS = new Set(
  String(process.env.LINK_CHECK_DEGRADED_HOSTS || "beian.miit.gov.cn")
    .split(",")
    .map((item) => item.trim().toLowerCase())
    .filter(Boolean),
);
...
function isDegradedHost(url) {
  try {
    return DEGRADED_HOSTS.has(new URL(url).hostname.toLowerCase());
  } catch {
    return false;
  }
}
...
async function checkLink(url) {
  const transientErrorPattern =
    /ECONNRESET|ETIMEDOUT|EAI_AGAIN|socket disconnected|socket hang up|ENOTFOUND/i;
  ...
  for (let attempt = 0; attempt <= RETRIES; attempt++) {
    ...
    const isTransientNetworkError =
      ...
      (typeof result.error === "string" &&
        transientErrorPattern.test(result.error));
...
async function main() {
  ...
  const linksToCheck = Array.from(links).filter(
    (link) => !ignored.some((prefix) => link.startsWith(prefix)),
  );
  console.log(
    `Checking ${linksToCheck.length} links with concurrency=${CONCURRENCY} ...`,
  );
  ...
  const results = await runWithConcurrency(
    ...
    async (url) => {
      ...
      const prefix =
        result.status === "ok" ? green(okLabel) : red(`FAIL (${reason})`);
      const method = result.method ? yellow(`[${result.method}]`) : "";
      const optionalInfo = result.optional ? " (optional host)" : "";
      const degradedHostInfo = result.degradedHost ? " (degraded host)" : "";
      const retryInfo =
        typeof result.retries === "number" && result.retries > 0
          ? ` (retries=${result.retries})`
          : "";
      console.log(
        `${method} ${url} -> ${prefix}${optionalInfo}${degradedHostInfo}${retryInfo}`,
      );
      return result;
    },
  );
